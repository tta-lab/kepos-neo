...
Kepos Neo 的 relay 必须保持：

- endpoint secret 永不进入 relay；
- Noise 加密从 peer 到 peer 保持端到端；
- relay admission 只决定是否提供带宽，不决定服务访问；
- relay allowlist 不能修改 publisher allowlist；
- relay 无法把未发布的 target 变成可达；
- relay 被攻破后，攻击者最多观察 metadata、消耗资源、降级或阻断连接。

直接 UDX、blind UDX relay 和未来 TCP/TLS 或 WSS relay 应共享同一授权边界。更换 carrier 不应改变谁有权访问服务。
...
用户选择一个已确认的人，发起一局游戏、一次远程协助或一个临时开发预览。活动结束后，session 权限随之结束。
